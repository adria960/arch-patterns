1.
The db tables can be created automatically - spring.jpa.hibernate.ddl-auto=create-drop
or use update.
The data are inserted with DataGenerator:
        Faker faker = new Faker();
        this.createUsers(faker);
        this.createProducts(faker);
        this.createOrder(faker);

-- DELETE - from materialized view, then
...
------------------------------------------------------------------------------------------------------------------------
2. The data are inserted with DataGenerator - first run - then comment out methods.
Or insert manually for step by step tests
...
-- or:
-- We could create a view purchase_order_summary to get the results we are interested in as shown here.
...
-- 3. FINAL SOLUTION - create MATERIALIZED  VIEW purchase_order_summary:
...
-- 8. FINAL - refresh in code - periodically executed
