...
-----------
  ...
  see images pgAdmin from 1 to 3.
    ...
    Connection:
        Host Name/Address: postgres            -- this is container name for postgres (started from docker)
        Port: 5432
...
--------------------------------------------------------------------------
    ...
    Run app with createOrder in DataGenerator commented - only users and products
    Stop app, comment out createUsers and createProducts and uncomment creatOrders!

    Users, products,, purchase_order tables are populated.

    I did not disable scheduling mat view refresh,
    but needed to execute it manually:
            REFRESH MATERIALIZED VIEW purchase_order_summary;
Now, use Postman to test app!
